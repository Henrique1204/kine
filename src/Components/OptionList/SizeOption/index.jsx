...
export function SizeOption({ value, text, onSelect, isChecked, isDisabled }) {
	...
	function generateStyle() {
		...
		if (isChecked) return 'bg-orange-700 text-white-100';
	...
	return (
		<span
			...
			className={`rounded flex-1 max-w-6 max-h-6 p-1 cursor-pointer bg-orange-500 hover:bg-orange-700 text-xs flex items-center justify-center ${generateStyle()}`}
		>
			{text}
		</span>
	);
